...
DASHBOARD_GROUPS = {
    ...
    "半導體/封測": ["2330.TW", "2337.TW"]
}

@st.cache_data(ttl=600)
def fetch_sidebar_dashboard():
    res = []
    all_t = [t for sub in DASHBOARD_GROUPS.values() for t in sub]
    try:
        df = yf.download(all_t, period="2d", progress=False, auto_adjust=True)['Close']
        if not df.empty:
            for name, stocks in DASHBOARD_GROUPS.items():
                valid = [s for s in stocks if s in df.columns]
                perf = ((df[valid].iloc[-1] / df[valid].iloc[-2]) - 1).mean() * 100
                res.append({"族群": name, "漲跌": f"{perf:+.2f}%", "趨勢": "🚀" if perf > 0.7 else ("➡️" if perf > -0.7 else "📉")})
    except: pass
