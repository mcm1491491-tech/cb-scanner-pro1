...
# 🔵 左側獨立面板：定義對齊玩股網與 Yahoo 的代表股
MAJOR_REF = {"半導體": ["2330.TW", "2454.TW"], "電子零組件": ["2317.TW", "2308.TW"], "建材營造": ["2542.TW", "2524.TW"], "電腦週邊": ["2382.TW", "2357.TW"], "航運類股": ["2603.TW", "2609.TW"]}
SUB_REF = {"PCB/載板": ["3037.TW", "2367.TW"], "AI/散熱": ["3017.TW", "3324.TW"], "矽智財/IP": ["3443.TW", "3661.TW"], "重電能源": ["1513.TW", "1519.TW"], "CoWoS/設備": ["3131.TW", "3583.TW"]}

@st.cache_data(ttl=300)
def fetch_independent_sidebar_data():
    """獨立抓取資料，對齊 5日(波段) 與 當日(即時)"""
    m_res, s_res = [], []
    all_t = list(set([t for l in list(MAJOR_REF.values()) + list(SUB_REF.values()) for t in l]))
    try:
        # 抓取 10 天數據，保證 5個交易日前 的基準點是對的
        df_market = yf.download(all_t, period="10d", progress=False, auto_adjust=True)['Close']
        if not df_market.empty:
            # 1. 大分類 (5日波段：今天 vs 5天前)
            for k, v in MAJOR_REF.items():
                sub = df_market[[t for t in v if t in df_market.columns]]
                perf = ((sub.iloc[-1] / sub.iloc[-6]) - 1).mean() * 100
                m_res.append({"大分類": k, "5日波段": f"{perf:+.2f}%", "趨勢": "📈 多頭" if perf > 1 else "➡️ 盤整"})
            # 2. 細分產業 (今日即時：今天 vs 昨收)
            for k, v in SUB_REF.items():
                sub = df_market[[t for t in v if t in df_market.columns]]
                perf = ((sub.iloc[-1] / sub.iloc[-2]) - 1).mean() * 100
                s_res.append({"細分產業": k, "今日即時": f"{perf:+.2f}%", "動態": "🚀 發動" if perf > 0.6 else "➡️ 震盪"})
    except: pass
...
with st.sidebar:
    ...
    df_m, df_s = fetch_independent_sidebar_data()
    ...
    if not df_s.empty: st.dataframe(df_s.style.map(lambda v: 'color: #00ff00' if '發動' in str(v) else '', subset=['動態']), hide_index=True)
    st.divider()
    selected_sector = st.selectbox("📁 選擇掃描族群", ["全部", "半導體業", "電腦及週邊設備業", "光電業", "建材營造", "電子零組件業", "其他"])
    conv_min, conv_max = st.slider("🎯 價值區間", 50, 200, (80, 125))
...
if st.session_state.df_main is not None:
    ...
    c2.metric("符合價值", len(filtered_df))
    ...
    if any(res.values()):
        ...
        st.download_button(label="📥 點我下載 Excel 完整報告", data=buf.getvalue(), file_name=f"CB報告_{datetime.now().strftime('%m%d')}.xlsx")
