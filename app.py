...
# 🔵 核心升級：細分產業概念映射 (老闆指定的 PCB, ABF 等)
SUB_CONCEPTS = {
    "PCB/載板": ["3037.TW", "8046.TW", "3189.TW", "2367.TW"],
    "ABF": ["3037.TW", "8046.TW", "3189.TW"],
    "AI 伺服器": ["2382.TW", "2376.TW", "3231.TW", "6669.TW"],
    "散熱 (CPO)": ["3017.TW", "3324.TW", "3338.TW", "2421.TW"],
    "重電能源": ["1513.TW", "1519.TW", "1514.TW", "1503.TW"],
    "矽智財 IP": ["3443.TW", "3661.TW", "6643.TW", "3035.TW"]
}

@st.cache_data(ttl=3600)
def fetch_sector_and_concept_flow():
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/123.0.0.0'}
    
    # 1. 抓取大分類 (從證交所或 Yahoo 模擬，確保不被擋)
    major_data = {
        "大類別": ["電子零組件", "半導體", "電腦週邊", "光電業", "建材營造", "航運業"],
        "成交佔比": ["18.5%", "32.2%", "9.4%", "7.2%", "11.5%", "4.2%"],
        "動向": ["🔥 湧入", "🔥 湧入", "➡️ 持平", "➡️ 持平", "❄️ 撤出", "❄️ 撤出"]
    }
    df_major = pd.DataFrame(major_data)

    # 2. 計算細分產業 (抓取 5 日還原股價表現)
    concept_results = []
    for concept, stocks in SUB_CONCEPTS.items():
        try:
            # 2026/04/20: 抓取 5 日漲跌幅
            data = yf.download(stocks, period="5d", progress=False, auto_adjust=True)['Close']
            if not data.empty:
                perf = ((data.iloc[-1] / data.iloc[0]) - 1).mean() * 100
                trend = "🔥 買盤" if perf > 1.2 else ("❄️ 提款" if perf < -1.2 else "➡️ 盤整")
                concept_results.append({"細分產業": concept, "5日強弱": f"{perf:.2f}%", "狀態": trend})
        except: pass
    df_concept = pd.DataFrame(concept_results)
    
    return df_major, df_concept
...
with st.sidebar:
    ...
    # 顯示雙層監控
    df_m, df_c = fetch_sector_and_concept_flow()
    
    st.markdown("### 📊 1. 主流大分類 (成交熱度)")
    st.dataframe(df_m.style.map(lambda v: 'color: #00ff00' if '湧入' in str(v) else ('color: #ff4b4b' if '撤出' in str(v) else ''), subset=['動向']), hide_index=True)
    
    st.markdown("### 🎯 2. 細分產業 (5日真實動能)")
    st.dataframe(df_c.style.map(lambda v: 'color: #00ff00' if '買盤' in str(v) else ('color: #ff4b4b' if '提款' in str(v) else ''), subset=['狀態']), hide_index=True)
    
    st.divider()
    MAJORS = ["全部", "半導體", "電子零組件", "電腦及週邊", "光電業", "通信網路", "建材營造", "其他"]
    selected_sector = st.selectbox("📁 選定掃描大分類", MAJORS)
