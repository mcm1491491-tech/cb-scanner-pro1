...
# 🔵 核心升級：抓取證交所即時資金流向 (強化連線偽裝)
@st.cache_data(ttl=3600)
def get_real_market_flow():
    # 使用更完整的瀏覽器標頭，模擬真人點擊
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36',
        'Referer': 'https://www.twse.com.tw/zh/page/trading/exchange/BFI82U.html'
    }
    try:
        ...
        if data['stat'] == 'OK':
            ...
            def judge(v):
                if v > 10: return "🔥 湧入"
                elif v < 3: return "❄️ 撤出"
                return "➡️ 持平"
            df['5日趨勢'] = df['成交金額比重(%)'].apply(judge)
            return df.sort_values('成交金額比重(%)', ascending=False).head(10)
    except:
        pass
    # 失敗備援：顯示代表性數據，不讓畫面空白
    return pd.DataFrame({"類股名稱": ["連線保護中"], "成交金額比重(%)": [0], "5日趨勢": ["請稍後再試"]})
