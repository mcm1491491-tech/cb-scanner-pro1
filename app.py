...
st.markdown("""
    ...
    div[data-testid="stTable"] th { background-color: #d4af37 !important; color: #0b0e14 !important; padding: 15px !important; border: 1px solid #d4af37; }
...
if uploaded_file:
    try:
        ...
        st.info(f"📊 已讀取 {len(df_cb)} 筆，符合條件共 {len(filtered_df)} 筆。")
        ...
        if st.button("🔥 啟動全自動雷達掃描"):
            ...
            for i, sym in enumerate(symbols):
                try:
                    ...
                    # 🔵 關鍵修正：修正 MultiIndex 格式
                    raw_df = yf.download(f"{sym}.TW", period="2y", progress=False)
                    if raw_df.empty:
                        raw_df = yf.download(f"{sym}.TWO", period="2y", progress=False)
                    
                    if len(raw_df) < 284: continue
                    
                    # 扁平化索引 (處理新版 yfinance 問題)
                    ...
                    m43 = float(df['MA43'].iloc[-1])
                    m87 = float(df['MA87'].iloc[-1])
                    m284 = float(df['MA284'].iloc[-1])
                    d43 = float(df['Close'].iloc[-43])
                    d87 = float(df['Close'].iloc[-87])
                    ...
                    # 抓取餘額比例 (如果找不到名稱就改抓第 7 欄)
                    bal_val = row.get('餘額比例', row.iloc[6])
                    balance = f"{bal_val:.2%}" if isinstance(bal_val, float) else str(bal_val)
        ...
        keys = ["top_right", "golden_cross", "mid_bull"]
